...
FIT框架提供了基于Hibernate Validator的数据校验功能，支持对方法参数进行自动校验。框架提供了两个版本的校验插件以适配不同的Java环境，与12.3的扩展校验实现不同在于，12.3只处理FIT框架提供的校验注解，而12.3则支持所有符合javax.validation规范或者jakarta.validation规范的校验注解：
